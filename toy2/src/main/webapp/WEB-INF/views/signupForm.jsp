...
<form name="signupForm" method="post" action="/saveDB">
    ...
    <div class="memberInfo">
        ...
        <div style="display: flex; align-items: center;">
            <input type="text" name="mbr_id" id="mbr_id" oninput="idCheck()" style="flex-grow: 1; margin-right: 5px;">
            <input type="button" value="중복확인" name="mbr_id" onclick="duplIdCheck()" style="flex-shrink: 0;">
        </div>
    ...
    <div class="memberInfo">
        <label for="email" style="display: block; font-weight: bold; margin-bottom: 5px;">이메일</label>
        <div style="display: flex; align-items: center;">
            <input type="email" name="email" id="email" placeholder="이메일" required style="flex-grow: 1; margin-right: 5px;">
            @
            <%--            직접입력 기능은 잠시 주석처리 --%>
            <%--            <input type="text" name="email2" id="email2" placeholder="직접입력">--%>
            <select name="email_etc" id="email_etc" style="margin-left: 5px; margin-right: 10px;">
        ...
        <button type="button" id="email_verification" style="padding: 8px 16px; background-color: #555; color: white; border: none; border-radius: 4px; margin-left: 5px;">인증하기</button>
    </div>
    <div class="memberInfo">
        <label for="addr_cd">주소</label>
        <div style="display: flex; align-items: center;">
            <input type="text" name="addr_cd" id="addr_cd" placeholder="우편번호" style="flex-grow: 1; margin-right: 5px;">
            <button type="button" style="flex-shrink: 0;">주소찾기</button>
        </div>
    ...
    async function signupTest() {
        ...
        */
        try {
            ...
            // 응답 본문은 한 번만 읽어올 수 있으므로 jsonResponse에 데이터를 담아서 쓴다.
            const jsonResponse = await response.json();

            // JSON에 담긴 Response 내용(= 에러메시지의 K,V값) 출력
            ...
            if (response.ok) {
                if (jsonResponse.status === 'success') {
                    ...
                    alert("response.status != succues 에러 발생. 관리자에게 문의하세요.");
                }
            } else {
                const errorText = Object.values(jsonResponse);
                alert("아래의 내용을 확인하여 제대로 입력해 주세요"+ "\t" + JSON.stringify(errorText, null, "\t"));
            }
        } catch (e) {
